...
public class Corpus
{
    ...
    public static Corpus load(int min, int max) throws IOException
    {
        Corpus corpus = new Corpus();
        for (int i = min; i < max; i = i + 2)
        {
        	corpus.addByDb(i, i + 2);
        }
        if (corpus.getVocabularySize() == 0) return null;

        return corpus;
    }
    public void addByDb(int min, int max)
    {
    	List<List<String>> wordsList = wordBuilder.getWordInRepositoryScope(min, max);
        for (List<String> words: wordsList)
        {
            this.addDocument(words);
        }
    }
