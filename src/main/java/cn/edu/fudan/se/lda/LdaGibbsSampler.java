...
public class LdaGibbsSampler {
     ...
     * �?后的模型个数（取收敛后的n个迭代的参数做平均可以使得模型质量更高）
     */
    ...
    public void initialState(int K) {
        ...
        z = new int[M][];   // z_i := 1到K之间的�?�，表示马氏链的初始状�??
        for (int m = 0; m < M; m++) {
     ...
     * 根据上述公式计算文档m中第n个词语的主题的完全条件分布，输出�?可能的主�?
     *
     * @param m document
     * @param n word
     */
    private int sampleFullConditional(int m, int n) {
        ...
        // scaled sample because of unnormalised p[] 正则�?
    ...
    public static double[] inference(double alpha, double beta, double[][] phi, int[] doc) {
        ...
        int N = doc.length;
        int[] z = new int[N];   // z_i := 1到K之间的�?�，表示马氏链的初始状�??
        for (int n = 0; n < N; n++) {
            int topic = (int) (Math.random() * K);
            z[n] = topic;
            // number of instances of word i assigned to topic j
